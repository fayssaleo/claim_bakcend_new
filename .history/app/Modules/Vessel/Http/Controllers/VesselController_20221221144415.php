...
class VesselController extends Controller
{
        ...
        if($request->id==0){
            ...
            if($request->shipping_line["id"]==0){
                if($request->shipping_line["name"]!=null || $request->shipping_line["name"]!=""){
                    $shippingLine_returnedValue=$this->shippingLine_confirmAndSave($request->shipping_line);
                    ...
                    $vessel->shipping_line_id=$shippingLine_returnedValue["payload"]->id;
                }
            }
            else{
                $shippingLine_returnedValue=$this->shippingLine_confirmAndUpdate($request->shipping_line);
    ...
    public function allClaim($claim_id){
        $vessel=Vessel::select()->where('claim_id', $claim_id)->with("typeOfEquipment")
        ->with("brand")
        ->with("natureOfDamage")
        ->with("department")
        //->with("estimate")
